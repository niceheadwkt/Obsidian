...
Profile instructions 跟個人化的 Skill 一樣，都會被 **頻繁讀進對話** ，每次新對話開始 Claude 都會讀一次。檔案太長有兩個副作用： **一是佔掉你的對話額度** ——你打的字、Claude 回的字，加上這份檔案，三者要共用同一個工作記憶空間，越長就越吃資源； **二是規則一多就容易自相矛盾** ，同一份檔案兩條規則打架，Claude 不知道該聽哪個。

Anthropic 並未公佈 profile instructions 的字數上限，但實測建議控制在 **500 字以內、4 到 6 段** ，這個長度剛好能寫清楚「我是誰／我要什麼／我不要什麼」，又不會塞滿到失焦。寫到一半發現超過，回頭砍掉抽象敘述、只留具體規則，判準是「這句話 Claude 看了會改變行為嗎？」若不會就刪。
...
打開 MCP 才能允許 Claude 最大程度地去利用第三方平臺的資料，例如 Chrome、Gmail、雲端硬碟、Notion 等等，是 AI 使用者絕對會用到的功能。
