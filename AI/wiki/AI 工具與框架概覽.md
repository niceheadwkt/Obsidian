...
**Cursor** 是一款基於 VS Code 分叉 (Fork) 開發的 AI 原生程式碼編輯器，已成為許多工程師的標準配備。
...
- **核心功能**：
  - **應用類型**：支援 Chatbot、智能體 (Agent，具備思考與工具調用能力)、以及圖形化工作流 (Workflow)。
  - **可視化 Canvas**：用拖拉黏貼的介面，將 Prompt、LLM、Python/Node.js 程式碼節點與 API 串聯。
  - **內建知識庫 (RAG)**：一鍵上傳 PDF、Notion 網頁，Dify 自動進行 Chunking 與 Embedding，無須手動架設向量資料庫。
  - **後端即服務 (BaaS)**：設計完成後，自動產出標準 API 接口與可直接分享的網頁 (WebApp)，大幅節省後端開發時間。
