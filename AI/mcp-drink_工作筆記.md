...
### 5. PWA 部署 Vercel 報 Python 版本錯誤 ── 冗餘檔案觸發自動 Serverless Functions 建置
*   **發生時間**：昨日 (2026-08-06)
...
### 6. 手機 PWA 側邊欄抽屜缺乏遮罩與點擊外部關閉功能 ── 行動端 PWA UI 自適應缺陷
*   **發生時間**：昨日 (2026-08-06)
...
### 7. 內置 AI 切換 Gemma 2 2B 報錯 ── 靜態宣告與 WebLLM 註冊表大小寫不一致
...
### 8. 小模型 (SLM) 在點餐意圖下的推理不穩定與幻覺 ── 參數量限制 (Prompt & Context Sensitivity)
...
### 9. 快取檢測未辨識 Qwen 2.5 3B ── OPFS 目錄名稱匹配干擾
...
### 10. 原生 Light 模式下選單「白底白字」看不見 ── 瀏覽器下拉選項 (Option) 繼承衝突
