# LLM Wiki Repository Instructions (AGENTS.md)
...
- **Wiki 目錄定義**：
  - 原始資料庫：[AI/sources/](AI/sources)（不可變、唯讀）。
  - 原始素材暫存區：[AI/raw/](AI/raw)（唯讀）。
  - 編譯後 Wiki：[AI/wiki/](AI/wiki)（由 AI 全權寫入、維護）。
  - 運作規範：[AI/schema.md](AI/schema.md)（所有 Agent 的行為準則）。
...
1. **資料匯入 (Ingest)**：
   - 使用者將原始檔案放置於 `AI/sources/` 或 `AI/raw/` 中。
   - 讀取檔案，提煉核心概念與關鍵實體。
   - 與使用者核對摘要重點。
   - 在 `AI/wiki/` 建立文獻總結頁面，並建立/更新關聯的概念與實體頁面（使用 `[[雙括號]]` 引用）。
   - 更新索引檔 [AI/wiki/index.md](AI/wiki/index.md)。
   - 在歷史日誌 [AI/wiki/log.md](AI/wiki/log.md) 中追加一筆：`## [YYYY-MM-DD] ingest | 標題`。

2. **知識查詢 (Query)**：
   - 讀取 [AI/wiki/index.md](AI/wiki/index.md) 定位相關頁面。
...
詳細運作規範與格式範例，請務必先閱讀並遵守 [AI/schema.md](AI/schema.md)。
